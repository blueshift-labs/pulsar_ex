defmodule PulsarEx.Connection do
  ...
  # ================== handle_call! =====================
  @impl true
  def handle_call({:create_producer, topic, opts}, {pid, _} = from, state) do
      ...
      :ok ->
        requests =
          Map.put(
            ...
            {from, pid, System.monotonic_time(:millisecond), request}
          )
  ...
  @impl true
  def handle_call({:subscribe, topic, subscription, sub_type, opts}, {pid, _} = from, state) do
    Logger.debug(
  ...
  def handle_info({:DOWN, ref, _, pid, _}, state) do
    ...
    requests = state.requests |> Enum.reject(&match?({_, {_, ^pid, _, _}}, &1))
      ...
      :ok ->
        requests =
          Map.put(
            requests,
            {:request_id, request.request_id},
            {nil, self(), System.monotonic_time(:millisecond), request}
          )
  ...
  defp handle_command(%CommandProducerSuccess{producer_ready: true} = response, _, state) do
    {{pid, _} = from, _, ts, request} =
      Map.get(state.requests, {:request_id, response.request_id})
  ...
  defp handle_command(%CommandProducerSuccess{} = response, _, state) do
    {_, _, ts, request} = Map.get(state.requests, {:request_id, response.request_id})
  ...
  defp handle_command(%CommandSuccess{} = response, _, state) do
    ...
    case request_info do
      {{pid, _} = from, _, ts, %CommandSubscribe{} = request} ->
        duration = System.monotonic_time(:millisecond) - ts
      ...
      {nil, _, ts, %CommandCloseProducer{producer_id: producer_id}} ->
        duration = System.monotonic_time(:millisecond) - ts
      ...
      {nil, _, ts, %CommandCloseConsumer{consumer_id: consumer_id}} ->
        duration = System.monotonic_time(:millisecond) - ts
  ...
  defp handle_command(%CommandError{error: err} = response, _, state) do
    ...
    case request_info do
      {from, _, ts, %CommandProducer{} = request} ->
        duration = System.monotonic_time(:millisecond) - ts
      ...
      {from, _, ts, %CommandSubscribe{} = request} ->
        duration = System.monotonic_time(:millisecond) - ts
  ...
  defp handle_command(%CommandAckResponse{request_id: request_id, error: nil}, _, state) do
    {{from, _, ts, request}, requests} = Map.pop(state.requests, {:request_id, request_id})
    state = %{state | requests: requests}
